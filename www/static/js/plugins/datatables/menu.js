...
$(function () {
  ...
  $('#register #btn').on('click', function () {
    ...
    if ($('#register').parsley().isValid() === true) {
      _addFun();
    }
    return false;
  });
  ...
  var table = $('[data-ride="datatables"]').DataTable({
    ...
    'aoColumnDefs': [
      ...
      },
      {
        'mRender': function (data, type, row) {
          if (data == 1) {
            return '<a href="javascript:void(0);" class="active" data-toggle="class" onclick="_chsta(0,' + row.id + ',2)">' +
              '<i class="fa fa-check text-success text-active"></i></a>';
          } else {
            return '<a href="javascript:void(0);" class="active" data-toggle="class" onclick="_chsta(1,' + row.id + ',2)"><i class="fa fa-times text-danger text-active"></i></a>';
          }
        },
        'bSortable': false,
        'aTargets': [7]
      },
      {
        'mRender': function (data, type, row) {
          if (data == 1) {
            return '<a href="javascript:void(0);" class="active" data-toggle="class" onclick="_chsta(0,' + row.id + ',1)"><i class="fa fa-check text-success text-active"></i></a>';
          } else {
            return '<a href="javascript:void(0);" class="active" data-toggle="class" onclick="_chsta(1,' + row.id + ',1)"><i class="fa fa-times text-danger text-active"></i></a>';
          }
        },
        'bSortable': false,
        'aTargets': [8]
      },
      {
        'mRender': function (data, type, row) {
          ...
          return '<a class="btn btn-xs" data-toggle="modal"' +
      ...
      {
        'mRender': function (data, type, row) {
          return '<input type="checkbox" name="post[]">';
        },
        'bSortable': false,
        'aTargets': [0]
      }
