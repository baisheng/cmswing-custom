module.exports = class extends think.common.AddonAdmin {
   ...
   */
  async indexAction () {
    this.active = 'addon/ad/admin/index'

    await this.clearCache()
    // 获取广告位置列表
    const data = await this.addonModel('ad_space').page(this.get('page')).order('spaceid DESC').countSelect();
    const html = this.pagination(data);
    this.assign('pagerData', html); // 分页展示使用
    this.assign('list', data.data);
    this.meta_title = '轮播图管理'
  ...
  async clearCache () {
    ...
    for (const v of space) {
      await this.addonModel('ad_space').upad(v.spaceid);
    }
  }
  /**
   * 更新广告缓存
  ...
  async addadAction () {
    if (this.isPost) {
      ...
      if (res) {
        // 更新广告位
        await this.addonModel('ad_space').upad(data.spaceid);
        // return this.fail(1)
        return this.success({name: '添加成功!', url: '/addon/ad/admin/adlist/?spaceid=' + data.spaceid});
      } else {
  ...
  async editadAction () {
    if (this.isPost) {
      ...
      if (res) {
        ...
        await this.addonModel('ad_space').upad(data.spaceid);
        // return this.fail(1)
        return this.success({name: '编辑成功!', url: '/addon/ad/admin/adlist/?spaceid=' + data.spaceid});
      } else {
  ...
  async tempAction () {
    // 获取广告位置列表
    const data = await this.addonModel('ad_temp').page(this.get('page')).countSelect();
    const html = this.pagination(data);
    this.assign('pagerData', html); // 分页展示使用
    this.assign('list', data.data);
    this.meta_title = '模板设置'
    return this.addonDisplay();
  }
  ...
  async addtempAction () {
    if (this.isPost) {
      ...
      if (res) {
        return this.success({name: '添加成功！', url: '/addon/ad/admin/temp'});
      } else {
        return this.fail('添加失败!');
      }
    } else {
  ...
  async edittempAction () {
    if (this.isPost) {
      ...
      if (res) {
        return this.success({name: '编辑成功！', url: '/addon/ad/admin/temp'});
      } else {
