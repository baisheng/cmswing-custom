...
module.exports = class extends Base {
  ...
  async indexAction() {
    ...
    // 加载页面头部底部钩子
    const editor = !think.isEmpty(info.editor) ? info.editor : await this.model('model').get_model(info.model_id, 'editor');
    const field_group = await this.model('model').get_model(info.model_id, 'field_group');
    const fields = await this.model('attribute').get_model_attribute(info.model_id, true);
