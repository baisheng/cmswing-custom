...
module.exports = class extends Admin {
  ...
  async __before () {
    ...
    this.extConfig = think.app.controllers.addons[`${c[1]}/config`];
    this.extPath = `${think.ROOT_PATH}/src/addons/${c[1]}`;
  ...
  async typeAction () {
    // 获取友情链接
    const data = await this.model('cx/ext_type').where({ext: this.ext.ext}).page(this.get('page')).order('sort ASC').countSelect();
    // console.log(data);
    ...
    if (think.isFile(sqlpath)) {
      ...
      try {
        for (let item of sqlarr) {
          item = item.trim();
          if (item) {
            item = item.replace(/cx_/g, this.config('model.mysql.prefix') || '');
