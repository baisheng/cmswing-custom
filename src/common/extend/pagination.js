...
module.exports = (pagerData, ctx, options) => {
  ...
  if (options.desc) {
    ...
    html += `<li class="page-item disabled"><a class="page-link">${total}</a></li>`;
  }
  if (currentPage > 1) {
    html += `<li class="page-item prev"><a class="page-link" href="${pageUrl.replace('__PAGE__', currentPage - 1)}">${options.text.prev}</a></li>`;
  }
  ...
  for (let i = 0, length = pageIndex.length; i < length; i++) {
    const p = pageIndex[i];
    if (p === currentPage) {
      html += `<li class="page-item active"><a class="page-link" href="${pageUrl.replace('__PAGE__', p)}">${p}</a></li>`;
    } else {
  ...
  if (currentPage < pagerData.totalPages) {
    html += `<li class="page-item next"><a class="page-link" href="${pageUrl.replace('__PAGE__', currentPage + 1)}">${options.text.next}</a></li>`;
  }
  return html;
};
