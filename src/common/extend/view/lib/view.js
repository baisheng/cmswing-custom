...
class View {
  ...
  parseFilePath (file, config = {}) {
    if (!file) {
      ...
      file = path.join(this.ctx.controller + config.sep + this.ctx.action)
    }
    const extRegExp = /\.\w+$/;
    ...
    if (!path.isAbsolute(file)) {
      ...
      file = path.join(think.ROOT_PATH + '/src/' + this.ctx.module, '/views', file);
    }
    return file;
  }
  ...
  render (file, config = {}) {
    ...
    config.modules = this.ctx.app.modules
