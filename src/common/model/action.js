...
module.exports = class extends think.Model {
  ...
  /**
   * 记录行为日志，并执行该行为的规则
   * await this.model("cx/action").log("action","model","record_id","user_id",this.ip(),this.http.url)
   ...
   */
  async executeAction (rules, action_id, user_id) {
    ...
    for (const rule of rules) {
      ...
      let step = parseInt(rule.rule, 10)
      let res;
      if (step >= 0) {
