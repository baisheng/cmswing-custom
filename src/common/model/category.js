...
module.exports = class extends think.Model {
  ...
  async exists (name, id) {
    let isExists
    if (!think.isEmpty(id)) {
      isExists = await this.model('category').where({id: ['!=', id], name: name}).find();
    } else {
      isExists = await this.where({name: name}).find()
    }
  ...
  async addCategory (data) {
    ...
    // 分类允许发布的内容类型
    // 默认为全类型（1目录，2主题，3段落）
    data.type = think.isArray(data.type) ? data.type.join(',') : data.type
