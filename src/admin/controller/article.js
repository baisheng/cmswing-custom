/* eslint-disable no-undef,guard-for-in,no-unused-expressions,prefer-reflect,max-depth,max-depth */
...
module.exports = class extends Base {
   ...
   */
  async indexAction () {
    this.active = `${this.ctx.module}/article/index`
    ...
    for (const val of list) {
      ...
      if (val.cover_id) {
        val.cover_id = await get_pic(val.cover_id, 1, 100)
      } else {
        val.pics = '/static/noimg.jpg';
      }
    }
   ...
   * @param integer $group_id 分组id
   */
  async _getDocumentList (cate_id, model_id, position, field, group_id, sortval, sortid) {
    // console.log(2222222);
    /* 查询条件初始化 */
    cate_id = cate_id || 0, field = field || true;
    const map = {};
  ...
  async getmenuAction () {
    ...
    if (!this.is_admin) {
      ...
      for (const val of cate) {
        ...
        val.target = '_self';
        delete val.icon;
        const priv = await this.model('category_priv').priv(val.id, this.roleid, 'init', 1);
        val.priv = priv;
        if (priv == 1 && val.pid != 0) {
          parr.push(val.pid);
        }
  ...
  async addAction () {
    ...
    for (let tag of tags) {
      tag.icon = await get_pic(tag.icon)
    }
    this.assign('icons', tags)
    // ~
    ...
    for (const key in parse_config_attr(model.field_group)) {
      for (const f of fields[key]) {
        ...
        } else if (f.type === 'htmlfile') {
          await this.hook('adminUpHtmlFile', f.name, 0, {$hook_key: f.name});
  ...
  async editAction () {
    ...
    let tags = await this.model('tags').select()
    for (let tag of tags) {
      tag.iconUrl = await get_pic(tag.icon)
    }
    this.assign('icons', tags)
    // 获取面包屑信息
    ...
    this.assign('model', model);
    const editor = !think.isEmpty(data.editor) ? data.editor : await this.model('model').get_model(data.model_id, 'editor');
    for (const key in parse_config_attr(model.field_group)) {
      for (const f of fields[key]) {
        if (f.type === 'editor') {
          // 添加编辑器钩子
          ...
          await this.hook('adminUpHtmlFile', f.name, data[f.name], {$hook_key: f.name});
        } else if (f.type === 'atlas') {
  ...
  async updateAction () {
    ...
    if (res) {
      // 行为记录
      if (!res.data.id) {
        // await this.model("common/action").log("add_document", "document", res.id, this.user.uid, this.ip(), this.http.url);
  ...
  async pasteAction () {
    ...
    if (!think.isEmpty(moveList)) { // 移动    TODO:检查name重复
      ...
      for (const v of copyList) {
        ...
        const table = await this.model('model').get_table_name(info.model_id);
        ...
        res = await this.model('document').updates(data);
      }
      await this.session('copyArticle', null);
      if (res) {
  ...
  async clearAction () {
    ...
    for (const v of clist) {
      // 模型表
      const table = await this.model('model').get_table_name(v.model_id);
      // console.log(table);
