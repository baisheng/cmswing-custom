...
module.exports = class extends Base {
  ...
  async logAction () {
    ...
    const map = {};
    map.status = ['>', -1];
    const list = await this.model('action_log').where({'status': ['>', -1]}).order('id DESC').page(this.get('page') || 1).countSelect()
    console.log(JSON.stringify(list))
    const html = this.pagination(list);
    console.log(html)
    this.assign('pagerData', html);
    for (const itme of list.data) {
      itme.action_id = await this.model('action').getAction(itme.action_id, 'title')
