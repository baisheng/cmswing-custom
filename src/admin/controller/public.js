/* eslint-disable no-undef,no-return-assign,guard-for-in,prefer-reflect */
...
module.exports = class extends Base {
  ...
  async signinAction () {
    ...
    // 用户登录
    const isLogin = await this._islogin();
    if (this.isAjax()) {
      // 验证码 钩子
      const signinBefore = await this.hook('signinBefore');
      if (!signinBefore) {
        // const error = think.picker.error
      ...
      const res = await this.model('member').signin(username, password, this.ip, 5, 1);
      if (res.uid > 0) {
        // 记录用户登录行为
        ...
        // const error = new
        // const error = this.controller(think.picker.error);
  ...
  async logoutAction () {
    // 退出登录
    const is_login = await this._islogin();
    if (is_login) {
